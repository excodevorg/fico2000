...
var FIN03151M00 = new FIN03151M00Component();
...
function FIN03151M00Component() {

};

FIN03151M00Component.prototype.ngOnInit = function () {
...
FIN03151M00Component.prototype.onSearch = function () {
        ...
        finService.getDecisionRatio(finAplyCom.alyid, finAplyCom.userid, finAplyCom.bzn, "Y","", FIN03151M00.decisionCallBack);
};


FIN03151M00Component.prototype.decisionCallBack = function (data) {
    ...
    if (data != null) {
        ...
        if (res.success) {
            ...
            finService.getFinRatio(finAplyCom.alyid, finAplyCom.userid, finAplyCom.bzn, "Y", "", FIN03151M00.finRateCallback);

            //FIN03151M00.decisionRatioTable();

            //finService.getFinRatio(finAplyCom.alyid, finAplyCom.userid, finAplyCom.bzn, lastest, enslDcd, FIN03151M00.finRateCallback);
...
FIN03151M00Component.prototype.onFinRateBaseTableView = function(idx, stacYyName) {
    ...
    bsTable += '<h3 class="panel-title">표준재무비율 (전체) <h3>';
    bsTable += '</div>';
    bsTable += '<div class="panel-body">';
    bsTable += '( 기준 년도 : ' +stacYyName+ ')';
    bsTable += '<table id="finRatioTable'+idx+'" class="table  table-condensed">';
    bsTable += '<thead>';
    bsTable += '<tr>';
    bsTable += '<th class="text-center">항목</th>';
    bsTable += '<th class="text-center">비율</th>';
    bsTable += '</tr>';
    bsTable += '</thead>';
    bsTable += '<tbody>';
    ...
    console.log('finRatioSet.length >> ', finRatioSet.length);
    console.log('size >> ', finRatioSet.length);

    if (rowStartCount == 2 || finRatioSet.length == size) {
        bsTable += '</div>';
    }
...
FIN03151M00Component.prototype.onFinRatioTableView = function(idx) {
...
bokRatioTable01 += "      <td align='right'>"+growthArray[i].bokAllRatio+" %</td>                                       ";
bokRatioTable01 += "  </tr>                                                                                                         ";    
}
...
bokRatioTable01 += "      <td align='right'>"+safeArray[i].bokAllRatio+" %</td>                                       ";
bokRatioTable01 += "  </tr>                                                                                                         ";    
}
...
bokRatioTable01 += "      <td align='right'>"+profitArray[i].bokAllRatio+" %</td>                                       ";
bokRatioTable01 += "  </tr>                                                                                                         ";    
}
...
bokRatioTable01 += "      <td align='right'>"+productArray[i].bokAllRatio+" %</td>                                       ";
bokRatioTable01 += "  </tr>                                                                                                         ";    
}
...
bokRatioTable01 += "      <td align='right'>"+activeArray[i].bokAllRatio+" 회</td>                                       ";
bokRatioTable01 += "  </tr>                                                                                                         ";    
}
...
bokRatioTable01 += " </tr>";
        ...
        $('#finRatioTable'+idx+' > tbody').html('');
        $('#finRatioTable'+idx+' > tbody:last').append(bokRatioTable01);
    };

FIN03151M00Component.prototype.finRateCallback = function(data) {
    ...
    if (data != null) {
        ...
        if (res.success) {
            ...
            for (var i = 0; i < decisionSet.stacYys.length; i++) {
                bsTable += FIN03151M00.onFinRateBaseTableView(i, decisionSet.stacYys[i].name);
            }
            ...
            for (var i = 0; i < decisionSet.stacYys.length; i++) {
                ...
                FIN03151M00.onFinRatioTableView(i);
...
FIN03151M00Component.prototype.onNumberWithCommas = function (totalAmt) {
    return numberWithCommas(totalAmt);
};

FIN03151M00Component.prototype.onFinResult = function () {
