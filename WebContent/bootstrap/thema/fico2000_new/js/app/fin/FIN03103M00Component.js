...
var FIN03103M00 = new FIN03103M00Component();
...
function FIN03103M00Component() {

};

FIN03103M00Component.prototype.ngOnInit = function () {
...
FIN03103M00Component.prototype.onSearch = function () {
        ...
        finService.getAnalysisFinDtTypeMaps(finAplyCom.alyid, finAplyCom.userid, finAplyCom.bzn, finAplyCom.stacYys, FIN03103M00.finAnalyCallback);


};


FIN03103M00Component.prototype.finAnalyCallback = function(data) {
    ...
    if (data != null) {
        ...
        if (res.success) {
            ...
            alyDataSet.forEach(function(selectedRaw, index) {
                ...
                FIN03103M00.onBalance(selectedRaw);
                FIN03103M00.onIncome(selectedRaw);
                FIN03103M00.onCost(selectedRaw);
                FIN03103M00.onAddedValue(selectedRaw);

                console.log('-------- ending -------');

                bsTable += FIN03103M00.onBsTableView(index);

                detTable += FIN03103M00.onDebtTableView(index);
            ...
            $("#roaTable").html(FIN03103M00.onRoaTableView()).trigger("create");
...
FIN03103M00Component.prototype.onBalance = function(res) {
...
FIN03103M00Component.prototype.onIncome = function(res) {
...
FIN03103M00Component.prototype.onCost = function(res) {
...
FIN03103M00Component.prototype.onAddedValue = function(res) {
    ...
    FIN03103M00.setAddValue();

};

FIN03103M00Component.prototype.setAddValue = function() {
...
FIN03103M00Component.prototype.getAddValue01 = function(accd, idx) {
...
FIN03103M00Component.prototype.getAddValueTotal = function(accd) {
...
FIN03103M00Component.prototype.getBsTotal = function(accd) {
...
FIN03103M00Component.prototype.getIncomeTotal = function(accd) {
...
FIN03103M00Component.prototype.getCostTotal = function(accd) {
...
FIN03103M00Component.prototype.onBsTableView = function(idx) {
	
    var num = Number(idx);
    var size = num + 1;
    var modValue = 0;

    FIN03103M00.getIncomeTotal('FAS0302010101'); //매출액 amslAmt
    FIN03103M00.getIncomeTotal('FAS0302010102'); //매출원가  ampmAmt
    FIN03103M00.getIncomeTotal('amslTtalPlAmt'); //매출순수익 = 매출액 - 매출원가 amslTtalPlAmt
    FIN03103M00.getIncomeTotal('FAS03020201'); //판매와관리비 slexExAmt
    FIN03103M00.getIncomeTotal('bsopPlAmt'); //영업손익금액 = 매출순수익 - 판매와관리비 bsopPlAmt
    FIN03103M00.getIncomeTotal('FAS03020301'); //영업외수익 nnoeAmt
    FIN03103M00.getIncomeTotal('FAS03020401'); //영업외비용 nnopExpAmt
    FIN03103M00.getIncomeTotal('crtxSbbfNtprAmt');//법인세 차감 전 순손익 crtxSbbfNtprAmt
    FIN03103M00.getIncomeTotal('FAS03020501'); //법인세비용 crtxExpAmt
    FIN03103M00.getIncomeTotal('crtrNtPlAmt'); //당기순이익 crtrNtPlAmt
    ...
    var i = 0;
    bsTable += '<tr>';
    bsTable += '<td>'+finAplyCom.stacYys[idx]+'</td>';
    bsTable += '<td class="text-right">'+FIN03103M00.onNumberWithCommas(amslAmt)+'</td>';
    bsTable += '<td class="text-right">'+FIN03103M00.onNumberWithCommas(nnoeAmt)+'</td>';i++;
    bsTable += '<td class="text-right">'+FIN03103M00.onNumberWithCommas(ampmAmt)+'</td>';i++;
    bsTable += '<td class="text-right">'+FIN03103M00.onNumberWithCommas(slexExAmt)+'</td>';i++;
    bsTable += '<td class="text-right">'+FIN03103M00.onNumberWithCommas(nnopExpAmt)+'</td>';i++;
    bsTable += '<td class="text-right">'+FIN03103M00.onNumberWithCommas(crtxExpAmt)+'</td>';i++;
    bsTable += '</tr>';
...
FIN03103M00Component.prototype.onDebtTableView = function(idx) {
    ...
    var debtable = "";
    var totalAmslAmt = amslAmt + nnoeAmt; //총수익
    var totalNnopExpAmt = nnopExpAmt + ampmAmt + slexExAmt + crtxExpAmt; //총비용
    var totalBon = FIN03103M00.getBsTotal('FAS030101') + FIN03103M00.getBsTotal('FAS030102'); //총자산

    console.log("FIN03103M00.getBsTotal('FAS030101') >>> " , FIN03103M00.getBsTotal('FAS030101'));
    console.log("FIN03103M00.getBsTotal('FAS030102') >>> " , FIN03103M00.getBsTotal('FAS030102'));

    var incomeRate = 0;
    var incomeRate1 = 0;
    var roaRate = 0;

    if (totalBon != 0 && totalBon != undefined && totalBon != null ) {
        incomeRate1 = amslAmt / totalBon;
        incomeRate1 = incomeRate1 * 100;
        incomeRate1 = incomeRate1.toFixed(2);
    } //총자산 회전율 
    
    if (amslAmt != 0 && amslAmt != undefined && amslAmt != null ) {
        incomeRate = crtrNtPlAmt / amslAmt;
        incomeRate = incomeRate * 100;
        incomeRate = incomeRate.toFixed(2); //매출액순이익율 
    } 

    if (incomeRate1 != 0 && incomeRate1 != undefined && incomeRate1 != null ) {
        roaRate = incomeRate * incomeRate1;
        //roaRate = roaRate * 100;
        roaRate = roaRate.toFixed(2);
    }

    var params = {
        key:finAplyCom.stacYys[idx],
        value:roaRate
    };

    roa_dataset.push(params);

    debtable += '<tr>';
    debtable += '<td >'+finAplyCom.stacYys[idx]+'</td>';
    
    debtable += '<td class="text-right">'+FIN03103M00.onNumberWithCommas(totalAmslAmt)+'</td>';
    
    debtable += '<td class="text-right">'+FIN03103M00.onNumberWithCommas(totalNnopExpAmt)+'</td>';
    debtable += '<td class="text-right">'+FIN03103M00.onNumberWithCommas(crtrNtPlAmt)+'</td>';
    debtable += '<td class="text-right">'+FIN03103M00.onNumberWithCommas(totalBon)+'</td>';
    debtable += '<td class="text-right">'+incomeRate+'</td>';
    debtable += '<td class="text-right" >'+incomeRate1+'</td>';
...
FIN03103M00Component.prototype.onRoaTableView = function() {
    var bsTable = "";
    bsTable += '<table class="table table-condense">';
    bsTable += '<thead>';
    bsTable += '<tr class="active">';
    bsTable += '<td>년도</td>';

    var start = Number(roa_dataset.length) - 1;
    
    for (var i = start; i >= 0; i--) {
        bsTable += '<td class="text-center">'+roa_dataset[i].key+'</td>';
    }
    bsTable += '</tr>';
    bsTable += '</thead>';
    bsTable += '<tbody>';
    bsTable += '<tr class="text-bold text-lg">';
    bsTable += '<td>총투자 수익율 (ROA)</td>';

    for (var i = start; i >= 0; i--) {
    bsTable += '<td class="text-right">'+roa_dataset[i].value+' %</td>';
    }
...
FIN03103M00Component.prototype.onPlot = function() {
...
console.log('-- onPlot >> ', start);

   for (var i = start; i >= 0; i--) {
    day_data.push({
        "elapsed":roa_dataset[i].key,
        "value":Number(roa_dataset[i].value)
        });
    }
...
FIN03103M00Component.prototype.onIncomeTableView = function(idx) {
	
    var num = Number(idx);
    var size = num + 1;
    var modValue = 0;

    var totalDebt = Number(FIN03103M00.getBsTotal('FAS030103')) + Number(FIN03103M00.getBsTotal('FAS030104')); //총부채
    var totalBon = FIN03103M00.getBsTotal('FAS030101') + FIN03103M00.getBsTotal('FAS030102'); //총자산
    var totalBs = FIN03103M00.getBsTotal('FAS030105'); //총자본
    
    FIN03103M00.getIncomeTotal('FAS0302010101');
    FIN03103M00.getIncomeTotal('FAS0302010102');
    FIN03103M00.getIncomeTotal('amslTtalPlAmt');
    FIN03103M00.getIncomeTotal('FAS03020201');
    FIN03103M00.getIncomeTotal('bsopPlAmt');
    FIN03103M00.getIncomeTotal('FAS03020301');
    FIN03103M00.getIncomeTotal('FAS03020401');
    FIN03103M00.getIncomeTotal('crtxSbbfNtprAmt');
    FIN03103M00.getIncomeTotal('FAS03020501');
    FIN03103M00.getIncomeTotal('crtrNtPlAmt');
    ...
    if (totalBon != 0 && totalBon != undefined && totalBon != null ) {
        incomeRate = crtrNtPlAmt / totalBon;
        incomeRate = incomeRate * 100;
        incomeRate = incomeRate.toFixed(2);


        incomeRate1 = totalBs / totalBon;
        incomeRate1 = incomeRate1 * 100;
        incomeRate1 = incomeRate1.toFixed(2);
    ...
    bsTable += '<tr>';
    bsTable += '<td>'+finAplyCom.stacYys[idx]+'</td>';
    bsTable += '<td class="text-right">'+FIN03103M00.onNumberWithCommas(totalBs)+'</td>';
    bsTable += '<td class="text-right">'+FIN03103M00.onNumberWithCommas(totalDebt)+'</td>';
    bsTable += '<td class="text-right">'+FIN03103M00.onNumberWithCommas(totalBon)+'</td>';
    bsTable += '<td class="text-right">'+FIN03103M00.onNumberWithCommas(crtrNtPlAmt)+'</td>';
    bsTable += '<td class="text-right">'+FIN03103M00.onNumberWithCommas(incomeRate)+'</td>';
    bsTable += '<td class="text-right">'+FIN03103M00.onNumberWithCommas(incomeRate1)+'</td>';
    bsTable += '</tr>';
...
FIN03103M00Component.prototype.onNumberWithCommas = function (totalAmt) {
    return numberWithCommas(totalAmt);
};

FIN03103M00Component.prototype.onFinResult = function () {
