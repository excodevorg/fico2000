...
var FIN03121M00 = new FIN03121M00Component();
...
function FIN03121M00Component() {

};

FIN03121M00Component.prototype.ngOnInit = function () {
...
FIN03121M00Component.prototype.onSearch = function () {
        ...
        finService.getDecisionRatio(finAplyCom.alyid, finAplyCom.userid, finAplyCom.bzn, "Y","", FIN03121M00.decisionCallBack);
};


FIN03121M00Component.prototype.decisionCallBack = function (data) {
    ...
    if (data != null) {
        ...
        if (res.success) {
            ...
            for (var i = 0; i < res.data.stacYys.length; i++) {
                bsTable += FIN03121M00.onDesisionBaseTableView(i, res.data.stacYys[i].name);
            }
            ...
            finService.getFinRatio(finAplyCom.alyid, finAplyCom.userid, finAplyCom.bzn, "Y", enslDcd, FIN03121M00.finRateCallback);

            //FIN03121M00.decisionRatioTable();

            //finService.getFinRatio(finAplyCom.alyid, finAplyCom.userid, finAplyCom.bzn, lastest, enslDcd, FIN03121M00.finRateCallback);
...
FIN03121M00Component.prototype.onDesisionBaseTableView = function(idx, stacYyName) {
...
FIN03121M00Component.prototype.decisionRatioTable = function(idx) {
...
console.log('decgrowthArray >> ', decgrowthArray);

    for (var i = 0; i < decgrowthArray.length; i++) {
    decisionRatioTable += "  <tr>															                                                ";
    decisionRatioTable += "      <td>"+decgrowthArray[i].itemNm+"</td>                                                                   ";
    if (decgrowthArray[i].finresult == 'BAD') decisionRatioTable += "<td style='cursor:pointer;'><span title='(기) "+growthArray[i].finRatio+" vs (한) "+growthArray[i].bokAllRatio+"' class='label label-table label-danger'>"+decgrowthArray[i].bokresult+"</span></td>";
    if (decgrowthArray[i].finresult == 'GOOD') decisionRatioTable += "<td style='cursor:pointer;'><span title='(기) "+growthArray[i].finRatio+" vs (한) "+growthArray[i].bokAllRatio+"' class='label label-table label-success'>"+decgrowthArray[i].bokresult+"</span></td>";
    decisionRatioTable += "  </tr>                                                                                                         ";    
    }
...
FIN03121M00Component.prototype.finRateCallback = function(data) {
    ...
    if (data != null) {
        ...
        if (res.success) {
            ...
            for (var i = 0; i < decisionSet.stacYys.length; i++) {
                ...
                FIN03121M00.decisionRatioTable(i);
...
FIN03121M00Component.prototype.onNumberWithCommas = function (totalAmt) {
    return numberWithCommas(totalAmt);
};

FIN03121M00Component.prototype.onFinResult = function () {
