...
var FIN03125M00 = new FIN03125M00Component();
...
function FIN03125M00Component() {

};

FIN03125M00Component.prototype.ngOnInit = function () {
...
FIN03125M00Component.prototype.onSearch = function () {
        ...
        finService.getDecisionRatio(finAplyCom.alyid, finAplyCom.userid, finAplyCom.bzn, "Y","", FIN03125M00.decisionCallBack);
};


FIN03125M00Component.prototype.decisionCallBack = function (data) {
    ...
    if (data != null) {
        ...
        if (res.success) {
            ...
            for (var i = 0; i < res.data.stacYys.length; i++) {
                bsTable += FIN03125M00.onDesisionBaseTableView(i, res.data.stacYys[i].name);
            }
            ...
            finService.getFinRatio(finAplyCom.alyid, finAplyCom.userid, finAplyCom.bzn, "Y", enslDcd, FIN03125M00.finRateCallback);

            //FIN03125M00.decisionRatioTable();

            //finService.getFinRatio(finAplyCom.alyid, finAplyCom.userid, finAplyCom.bzn, lastest, enslDcd, FIN03125M00.finRateCallback);
...
FIN03125M00Component.prototype.onDesisionBaseTableView = function(idx, stacYyName) {
...
FIN03125M00Component.prototype.decisionRatioTable = function(idx) {
    ...
    if (decactiveArray[i].finresult == 'BAD') decisionRatioTable += "<td style='cursor:pointer;'><span title='(기) "+activeArray[i].finRatio+" vs (한) "+activeArray[i].bokAllRatio+"' class='label label-table label-danger'>"+decactiveArray[i].bokresult+"</span></td>";
    if (decactiveArray[i].finresult == 'GOOD') decisionRatioTable += "<td style='cursor:pointer;'><span title='(기) "+activeArray[i].finRatio+" vs (한) "+activeArray[i].bokAllRatio+"' class='label label-table label-success'>"+decactiveArray[i].bokresult+"</span></td>";
    decisionRatioTable += "  </tr>                                                                                                         ";    
    }
...
FIN03125M00Component.prototype.finRateCallback = function(data) {
    ...
    if (data != null) {
        ...
        if (res.success) {
            ...
            for (var i = 0; i < decisionSet.stacYys.length; i++) {
                ...
                FIN03125M00.decisionRatioTable(i);
...
FIN03125M00Component.prototype.onNumberWithCommas = function (totalAmt) {
    return numberWithCommas(totalAmt);
};

FIN03125M00Component.prototype.onFinResult = function () {
