...
@Component("QsrtMngNoRslt_20170130220902137000")
public class QsrtMngNoRslt_20170130220902137000 extends QsrtInfoConsult{
	
	private static Log logger = LogFactory.getLog(QsrtMngNoRslt_20170130220902137000.class);
		...
		for (QsrItemInfoDomain qsrItemVo : qsrtItemList) {
			...
			switch (lprbmNo) {
				case 0 :
					...
					//결과값 셋팅
					qsrtDtl.setRsltVl(this.qsrtInfoDtlVo.getStacYy()); 
					break;
				case 1 :
					//1) 문항
				...
				case 4 :
					...
					List<QsrtInfoDtlVo> arrList01 = (ArrayList<QsrtInfoDtlVo>) prbmNo.get("no"+sprbmNo);
					
					QsrtInfoDtlVo vo01 = null;
					
					vo01 = (QsrtInfoDtlVo) beanUtil.toCopy(qsrtDtl, new QsrtInfoDtlVo());
					vo01.setKindNm("판매관리비");
					vo01.setRsvlType("0");
					vo01.setRsltVl("0");
					vo01.initial();
					arrList01.add(vo01);
				...
				case 9 :
					...
					setQsrtPrbmItem(sprbmNo, prbmNo, qsrItemVo, qsrtDtl);
					String today = FasDateUtil.getCurrentDay().replaceAll("-", "");
					if (sprbmNo == 1) { //1개월전
						String ym = this.qsrtInfoDtlVo.getStacYy() + "10";
						qsrtDtl.setBaseYm(ym);
					} else if (sprbmNo == 2) { //2개월전
						String ym = this.qsrtInfoDtlVo.getStacYy() + "11";
						qsrtDtl.setBaseYm(ym);
					} else if (sprbmNo == 3) { //3개월전
						String ym = this.qsrtInfoDtlVo.getStacYy() + "12";
						qsrtDtl.setBaseYm(ym);
					}
				...
				case 10 :
					...
					setQsrtPrbmItem(sprbmNo, prbmNo, qsrItemVo, qsrtDtl);
					List<QsrtInfoDtlVo> arrList = (ArrayList<QsrtInfoDtlVo>) prbmNo.get("no"+sprbmNo);
					
					QsrtInfoDtlVo vo = null;
					
					for (int i = 1; i <= 12; i++) {
						vo = (QsrtInfoDtlVo) beanUtil.toCopy(qsrtDtl, new QsrtInfoDtlVo());
						String ym = this.qsrtInfoDtlVo.getStacYy() + StringUtil.lpad(i, 2);
						vo.setBaseYm(ym);
						vo.setRsvlType("0");
						vo.setRsltVl("0");
						vo.initial();
