...
//노동소득분배율
public class FinRatio911  extends FinRatio {
	...
	public void cal() {
		...
		for (int idx = startIdx; idx < size; idx++) {
			...
			try {
				beforeAmt = beforeFin.get(FinCostConst.노무비).getValue().add(afterFin.get(FinIncomeItemConst.급여).getValue());
				afterAmt = afterFin.get(FinAddedValueItemConst.부가가치합계).getValue();
