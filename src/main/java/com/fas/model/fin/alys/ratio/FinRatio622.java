...
//차입금평균이자율
public class FinRatio622 extends FinRatio {
	...
	public void cal() {
		...
		for (int idx = startIdx; idx < size; idx++) {
			...
			afterAmt = afterAmt.add(afterFin.get(FinBalanceItemConst.회사채).getValue());
			afterAmt = afterAmt.add(afterFin.get(FinBalanceItemConst.장기금융기관차입금).getValue());
			afterAmt = afterAmt.add(afterFin.get(FinBalanceItemConst.단기금융기관차입금).getValue());
