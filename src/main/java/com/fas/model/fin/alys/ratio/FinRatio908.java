...
//설비투자효율
public class FinRatio908  extends FinRatio {
	...
	public void cal() {
		...
		for (int idx = startIdx; idx < size; idx++) {
			...
			try {
				...
				afterAmt = afterFin.get(FinBalanceItemConst.유형자산합계).getValue().subtract(afterFin.get(FinBalanceItemConst.건설중인자산).getValue());
