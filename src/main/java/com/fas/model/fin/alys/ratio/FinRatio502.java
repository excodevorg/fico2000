...
public class FinRatio502 extends FinRatio {
	@Override
	public void cal() {
		...
		for (int idx = startIdx; idx < size; idx++) {
			...
			beforeAmt = beforeAmt.add(beforeFin.get(FinBalanceItemConst.유형자산합계).getValue());
			
			afterAmt = afterAmt.add(afterFin.get(FinBalanceItemConst.유형자산합계).getValue());
